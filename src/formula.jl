...
function allvars(ex::Expr)
    ...
    [[allvars(a) for a in ex.args[2:length(ex.args)]]...]
end
...
function dospecials(ex::Expr)
    ...
    excp = copy(ex)
    excp.args = vcat(a1,map(dospecials, ex.args[2:length(ex.args)]))
    if a1 != :* return excp end
    aa = excp.args
    a2 = aa[2]
    a3 = aa[3]
    if length(aa) > 3
        excp.args = vcat(a1, aa[3:length(aa)])
        a3 = dospecials(excp)
    end
    :($a2 + $a3 + $a2 & $a3)
end
...
function ex_or_args(ex::Expr,s::Symbol)
    ...
    excp = copy(ex)
    a1 = ex.args[1]
    a2 = map(condense, ex.args[2:length(ex.args)])
...
function condense(ex::Expr)
    ...
    excp = copy(ex)
    excp.args = vcat(a1, map(x->ex_or_args(x,a1), ex.args[2:length(ex.args)])...)
    excp
end    
condense(a) = a

getterms(ex::Expr) = (ex.head == :call && ex.args[1] == :+) ? ex.args[2:length(ex.args)] : ex
getterms(a) = a
...
function evt(ex::Expr)
    ...
    filter(x->!isa(x,Number), vcat(map(getterms, ex.args[2:length(ex.args)])...))
end
evt(a) = {a}
